...
test.describe("Public Portfolio E2E Suite", () => {
  test("should handle non-existent portfolio gracefully with 404 or error boundary", async ({ page }) => {
    ...
    const errorOrNotFoundText = page
      .locator("text=/404|not found|Page Error|Something went wrong|Connection Issue|Try Again/i")
      .first();
    await expect(errorOrNotFoundText).toBeVisible({ timeout: 15000 });
  });
});
